...
const DuplicateSmartphone = () => {
  ...
  const { data } = result || {};
  const [duplicateSmartphone, dupResult] = useDuplicateSmartphoneMutation();
  const {
    register,
    handleSubmit,
    setValue,
  ...
  const handleDuplicateSubmit = (phoneData: ISmartPhone) => {
  ...
  useEffect(() => {
    if (dupResult.isSuccess) {
      toast({
        duration: 3000,
        variant: "success",
        description: dupResult?.data?.message,
      });
      setTimeout(() => {
    ...
    if (dupResult.isError) {
      toast({
        duration: 3000,
        // @ts-ignore
        description: dupResult.error?.message,
        variant: "error",
      });
    }
  }, [dupResult.isSuccess, dupResult.isError]);
          ...
          <div className="p-8 pb-20">
            ...
            <form onSubmit={handleSubmit(handleDuplicateSubmit)}>
              <div className="flex flex-col sm:grid grid-cols-2 sm:grid-cols-4 gap-x-6 lg:gap-x-10 gap-y-5 mt-8 max-w-screen-sm mx-auto smartphoneForm  justify-start items-start">
                ...
                <div className="grid w-full items-center gap-1.5 col-span-4">
                  <Label
                    htmlFor="duplicate_aboutThisPhone"
                    className="text-gray-500"
                  >
                    About This Phone
                  </Label>
                  <Textarea
                    id="duplicate_aboutThisPhone"
                ...
                <div className="grid w-full items-center gap-1.5 col-span-2">
                  <Label
                    htmlFor="duplicate_releasedDate"
                    className="text-gray-500"
                  >
                    Release Date
                  </Label>
                ...
                <Button
                  ...
                  loading={dupResult.isLoading}
                  disabled={dupResult.isLoading}
                >
                  Duplicate Now
                </Button>
              </div>
